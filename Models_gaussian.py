...
class LaplacePolicy(nn.Module):	
	...
	def forward(self, state, goal):	
		...
		mean5 = self.hierarchies_selected[:, 0].unsqueeze(dim=1) * mean5
		mean4 = self.hierarchies_selected[:, 1].unsqueeze(dim=1) * mean4
		mean3 = self.hierarchies_selected[:, 2].unsqueeze(dim=1) * mean3

		scale5 = self.hierarchies_selected[:, 0].unsqueeze(dim=1) * scale5
		scale4 = self.hierarchies_selected[:, 1].unsqueeze(dim=1) * scale4
		scale3 = self.hierarchies_selected[:, 2].unsqueeze(dim=1) * scale3

		means = [mean2, mean3, mean4, mean5]
		stds = [scale2, scale3, scale4, scale5]
		variances = [std**2 for std in stds]

		N = ((self.hierarchies_selected[:, 0] +  self.hierarchies_selected[:, 1] +  self.hierarchies_selected[:, 2]).reshape(2048,1).expand(2048,31) + 1e-7)
		#N = len(means)   #decide what you want 
		mean_combined = sum(means) / N
		variance_combined = (sum(variances) / N) + (sum((mean - mean_combined)**2 for mean in means) / N)
		std_combined = torch.sqrt(torch.tensor(variance_combined))
		combined_distribution = torch.distributions.normal.Normal(mean_combined, std_combined)
